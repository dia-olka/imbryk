...
| `ENABLE_QUALITY_GRADING`          | Env var | `false`                   | Enable Quality Grading — LLM-as-judge scores each newspaper on voice, coherence, originality, depth, headlines, prompt coverage. Scores are embedded in article metadata and fed back into editorial reflections and generation prompts. |
| `QUALITY_GATE_THRESHOLD`          | Env var | `2.5`                     | Minimum overall quality score (1-5) to pass the quality gate. Articles scoring below this are regenerated with the critique injected (max 1 retry per newspaper). |
